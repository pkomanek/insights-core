...
The following combienrs are included in this module:
...
class PackageProvides(dict):
    ...
    def get_package(self, command):
        """
        .. warn::
            This combiner class is deprecated, please use the built-in `get()`
            of the `dict` instead.

        """
        deprecated(
            PackageProvides.get_package,
            'Please use the `get()` instead.'
        )
...
class PackageProvidesHttpdAll(PackageProvides):
    ...
    Examples:
        ...
        >>> httpd_packages.get_package("/usr/sbin/httpd")
